...
from PyQt5.QtGui import QIcon, QPixmap, QKeySequence, QPainter, QPen, QBrush, QColor
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.LeftButton:
            ...
            if self.pickColor:
                ...
                self.colorLabel.setStyleSheet(f"background-color:{hex_}; border:2px solid #000000;border-radius: 10px;")
                self.pickColor = False
...
class Window(qt.QMainWindow):
    def __init__(self, superClass, filename, _type, paintOptionSelected):
        super().__init__()
    ...
    def closeEvent(self, event):
        ...
        if self.imgOption == "close":
            ...
            if reply == qt.QMessageBox.No:
                ...
                self.paintOptionSelected(self.type, self.imgOption)
            ...
            self.canvas.pixmap().save("images/temp/foo2.png")
            self.paintOptionSelected(self.type, self.imgOption)
            self.superClass.show()
    ...
    def openColorPicker(self):
        ...
        if color.isValid():
            ...
            self.colorLabel.setStyleSheet(f"background-color:{colorName}; border:2px solid #000000;border-radius: 10px;")
