...
from PyQt5.QtGui import QFont, QTextCharFormat, QPalette, QColor, QIcon, QPixmap
...
class Window(qt.QMainWindow):
    ...
    def updateScoreLabels(self):
        ...
        if notesExist:
            ...
            if noteData:
                ...
                averageScore = round(100*sum(total) / len(total))
    ...
    def showRadarChart(self, scores):
        ...
        plt.yticks([3, 5, 8], ["3", "5", "8"], color="grey", size=12)
        plt.ylim(0, 10)
        ...
        pixmap = pixmap.scaled(QSize(450, 300), transformMode=Qt.SmoothTransformation)
    ...
    def changeGraph(self, i):
        ...
        label = qt.QLabel(self)
        label.setAlignment(Qt.AlignHCenter)
        label.setPixmap(self.imageList[self.topicListIndex])
        self.chartLayout.addWidget(label)
    ...
    def navigateToLearn(self):
        ...
        if notes:
            self.w = learnFilter.Window(self, self.courseName, self.color)
            self.w.show()
            self.hide()
        else:
